...
'use strict';
  ...
  function postTabs (args, content) {
    ...
    for (var i = 0; i < matches.length; i += 2) {
      ...
      var isActiveNav = (tabId == 1) ? 'true' : 'false';
	  var isActiveCon = (tabId == 1) ? ' show' : '';
	  tabNav += '<a class="btn btn-primary" role="button" data-toggle="collapse" href="#' + tabHref + '" aria-expanded="' + isActiveNav + '" aria-controls="' + tabHref + '">' + tabIcon + tabCaption + '</a> ';
      tabContent += '<div class="collapse' + isActiveCon + '" data-parent="#' + tabName.toLowerCase().split(' ').join('-') + '" id="' + tabHref + '">' + postContent + '</div>';
    }
    tabNav = '<p>' + tabNav + '</p>';
    tabContent = '<div>' + tabContent + '</div>';
    return '<div class="container" id="' + tabName.toLowerCase().split(' ').join('-') + '">' + tabNav + tabContent + '</div>';
  }
